...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Anarcoin.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: Anarcoind.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeignitionamount=<n> " + _("Keep N Anarcoin anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Anarcoin is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Anarcoin is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Anarcoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Anarcoin server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Anarcoin") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Anarcoin to complete") << "\n";
    ...
    nAnonymizeAnarcoinAmount = GetArg("-anonymizeignitionamount", 0);
    if(nAnonymizeAnarcoinAmount > 999999) nAnonymizeAnarcoinAmount = 999999;
    if(nAnonymizeAnarcoinAmount < 2) nAnonymizeAnarcoinAmount = 2;
    ...
    LogPrintf("Anonymize Anarcoin Amount %d\n", nAnonymizeAnarcoinAmount);
